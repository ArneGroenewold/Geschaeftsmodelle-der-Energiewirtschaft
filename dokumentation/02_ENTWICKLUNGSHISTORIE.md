...
- **Ausbau (gleiche Session):** Auftraggeber wählte **eine Mission pro Modul (8 gesamt)**. 7 weitere Missionen ausgeschrieben — je 6 Entscheidungen, 3 thematisch zugeschnittene Dimensionen, 4 Ergebnisstufen: SZ-HANDEL-DESK (M1, Handelstisch: Ertrag/Risiko/Compliance), SZ-BESS-MERCHANT (M2, Batteriespeicher: Rendite/Optimierung/Resilienz), SZ-VNB-KONZESSION (M3, Verteilnetz: Wirtschaftlichkeit/Versorgungssicherheit/Regulatorik), SZ-FLEX-AGGREGATOR (M5, VPP: Marge/Portfolio/Resilienz), SZ-SOLAR-SERVICE (M6, Solar-as-a-Service: Wirtschaftlichkeit/Wachstum/Bilanz-Resilienz), SZ-H2-HOCHLAUF (M7, grüner Wasserstoff: Wirtschaftlichkeit/Abnahmesicherheit/Resilienz), SZ-DEV-TO-SELL (M8, Projektentwickler: Rendite/Pipeline/Risiko).
- **Verifikation:** Struktur-Check (8 Missionen × 6 Schritte, alle `punkte`-Keys ∈ `dimensionen`, je 3 Optionen, Debrief vorhanden — 0 Probleme) + Runtime-Durchlauf aller 8 Missionen durch die Engine (Debrief mit 3 Balken + Stufe, keine Fehler) + Home zeigt 8 Karten, keine Konsolenfehler. `sw.js` CACHE_NAME v14 → v15.

### Session 29 — Backlog angelegt & Doku-Konsolidierung (Juli 2026)
- **Auftrag:** Weiterentwicklung vorerst abschließen; sicherstellen, dass der Projektordner alle Informationen enthält, um Wiki *und* Lern-App künftig weiterzuentwickeln. Backlog mit Weiterentwicklungsideen und inhaltlichen Lücken anlegen.
- **Neu: `09_BACKLOG.md`** — eine einzige priorisierte Liste (P1/P2/P3) für **beide** Anwendungen, statt Offenes über README/Projektkontext/Konzepte verstreut zu lassen. Enthält: gemessene inhaltliche Lücken, Wiki-Backlog, Lern-App-Backlog, Übergreifendes/Betrieb und einen Abschnitt **„Bewusst verworfen / nicht wieder aufmachen"** (Dark Mode, Build-Step, Faktenduplizierung, BMC für Institutionen, Vollrollout ohne Abstimmung).
- **Inhaltliche Lücke gemessen statt geschätzt:** Skript über `GM_DATA` × `LERN_QUIZ_ITEMS`/`LERN_UNITS` ergab: von **75 Steckbriefen sind 64 abgedeckt, 11 kommen in der Lern-App überhaupt nicht vor** (u.a. `L3-RECHT-KANZLEI-01` Energierechtskanzlei, `L3-VERSICH-ENERGIE-02` Cyber-Versicherung/KRITIS, `L3-BERAT-STRAT-02` IT-/Digitaltransformation — ausgerechnet das eigene Beratungssegment). Liste + wiederverwendbares Prüfskript im Backlog §1.1. Weiterer Befund: **BMC-Puzzle nutzt nur 11 von 73 möglichen Steckbriefen**, obwohl das Format authoring-frei ist.
- **Doku-Sync:** `README.md` (Dateibaum um `lern-szenarien.js`/`szenario-engine.js` + Backlog ergänzt, Wegweiser-Zeile, Lern-App-Stand S26→S28, neuer Backlog-Abschnitt), `08_LERNAPP_UMSETZUNG.md` (Roadmap: Szenario-Missionen erledigt, Verweis aufs Backlog, **Anleitung „neue Szenario-Mission anlegen"**), `00_PROJEKTKONTEXT.md` (v2.7-Zeile in der Historie, Lern-App-Stand korrigiert — stand noch auf „MVP Modul 4, 54 Items" —, Backlog-Verweise).
- **Zwei stolperfallen-relevante Korrekturen in `00_PROJEKTKONTEXT.md` §7:** (1) Die Anweisung „Syntax via Node prüfen" ging ins Leere — **auf dem Rechner sind weder Node noch Python installiert** (`python` ist nur der Store-Stub); stattdessen Browser-Prüfung dokumentiert, inkl. Hinweis, dass die Lern-App wegen des Service Workers HTTP braucht (PowerShell-`HttpListener` als Workaround). (2) Neu dokumentiert: **vor jedem Lern-App-Test die SW-Caches leeren**, sonst testet man die alte Version — das hat in Session 27 zunächst zu einem falschen Negativbefund geführt.
- **Offen geblieben (bewusst):** „1,5 Grad"-Namensfrage — der String existiert nicht im Code, „Fünf Grad" und „1KOMMA5°" sind korrekt; im Backlog unter „bewusst verworfen" festgehalten, bis der Auftraggeber die Fundstelle nennt.
